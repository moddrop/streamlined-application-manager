...
let imagerOffsetX, imagerOffsetY, imagerIsDown = false;

imager.addEventListener('mousedown', (e) => {
    imagerIsDown = true;
    imagerOffsetX = e.clientX - imager.offsetLeft;
    imagerOffsetY = e.clientY - imager.offsetTop;
});

document.addEventListener('mouseup', () => { imagerIsDown = false; });
document.addEventListener('mousemove', (e) => {
    if (!imagerIsDown) return;
    reminder1.style.left = (e.clientX - reminder1OffsetX) + 'px';
    reminder1.style.top = (e.clientY - reminder1OffsetY) + 'px';
});	
